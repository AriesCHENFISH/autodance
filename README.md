...
- 上传固定机位的 MP4 舞蹈练习视频；
- 使用 Ultralytics YOLO Pose 检测人物和人体关键点；
- 使用 ByteTrack（默认）或 BoT-SORT 分配并维持人物 ID；
...
这里的 `x`、`y` 是当前 Phase 的视频像素坐标。人物暂时被遮挡、检测置信度不足或尚未获得追踪 ID 时，该帧的 `persons` 可能为空。
