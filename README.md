...
**Phase 3：关键队形检测**，以及 **Phase 4：队形编辑和 SVG 导出**。
暂不分析动作、姿态类别、手势或舞步。
...
- 自动将 Phase 3 结果载入队形编辑器，也支持单独载入已有 `formations.json`；
- 支持队形选择、名称和时间修改、人物格位校正、复制和删除队形；
- 生成自包含的平面队形 SVG，并导出编辑后 JSON、当前 SVG 和全部 SVG ZIP；
- 将异常写入 `analysis.log`，单帧失败时继续处理后续画面。

PDF 编排和导出属于后续 Phase，当前版本尚未实现。
...
视频分析完成后，Phase 4 编辑器会自动载入本次 `formations.json`；也可以在
不重新分析视频的情况下手动载入任意已验证的 `formations.json`。

编辑流程：

1. 在“当前队形”中选择要校正的队形；
2. 修改队形名称或展示时间；
3. 在人物下拉框选择 ID，然后点击画布上的目标格点；
4. 如需精确调整，可直接修改人物坐标表的 X、Y；
5. 点击“应用修改”后再切换到其他队形；
6. 可复制当前队形作为新增队形，也可删除误检队形；
7. 点击“保存并导出 SVG”。

坐标会吸附到最近的整数格点，并严格限制在当前队形网格范围内。人物 ID
必须唯一；越界坐标、重复 ID、空名称和负时间都会被拒绝。至少保留一个队形。

每次导出会创建：

```text
data/outputs/<时间_任务ID>/
├── formations_edited.json
├── formation_001.svg
├── formation_002.svg
├── ...
└── formation_svgs.zip
...
`formations_edited.json` 保留 Phase 3 的来源字段，并增加 `name`；被修改或
复制的队形还会包含 `"edited": true`。SVG 使用与 `formations.json` 相同的
前后场方向、人物 ID 和格位坐标，可直接交给后续 PDF 阶段使用。
...
├── visualization/
│   ├── editor.py           # 队形编辑状态和点击画布
│   ├── svg_generator.py    # SVG 渲染、校验与批量导出
...
1、罢免视频识别id和动线的思路，改成识别有效队形的帧，然后重新输入系统去识别人物id，否则两个不同人物重叠后id很有可能混乱。

2、可以，现在人物ID和队形识别都完成得较好。现在出现了新的问题：舞台由人工标定的话，容易不对称、不准确。同时由于人跳舞还是会有走位误差的，所以识别出来的队形可能略有不对称。但是我希望舞台标定由计算自动标定，因为上传的练习室视频往往是固定机位的正视角。生成出来的svg队形，我也希望在分析后尽可能对称/规整。但是有的队形它不一定是左右对称，也可能中心对称（尤其是偶数队形时），我不知道后面这个问题有没有现有的算法来解决。

3、标定矩形选择图中能承载的最大的矩形，否则无法把人全都框进去。对称问题再修改。
